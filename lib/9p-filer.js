...
function Virtio9p(filesystem, bus) {
    ...
    this.fids = {};
    // Any inflight responses might get flushed before we complete the i/o.
    // This is a list of all valid inflight responses that can continue.
    // If any aysnc i/o callback happens, and its pendingTag is missing, it can abort.
...
Virtio9p.prototype.get_state = function() {
    ...
    // TODOhumphd: need to confirm my fids work like this...
...
Virtio9p.prototype.ReceiveRequest = function (index, GetByte) {
    ...
    switch(id) {
        ...
        case P9_TLOPEN:
            ...
            // TODO: should probaby check mode and do proper open()
            self.fs.stat(path, function (err, stats) {
                ...
                message.Debug("file opened path=" + path);
                req[0] = stats.qid;
                req[1] = self.msize - 24;
    ...
    //                self.fids[fid] = self.Createfid(newDir, FID_INODE, uid);
                    marshall.Marshall(["Q"], [stats.qid], self.replybuffer, 7);
...
//            };
            self.fs.open(newFilePath, 'w', function(err, fd) {
                ...
                self.fs.fstat(fd, function(err, stats) {
                    ...
                    marshall.Marshall(["Q", "w"], [stats.qid, self.msize - 24], self.replybuffer, 7);
                    self.BuildReply(id, tag, 13+4);
                    self.SendReply(0, index);    
                });
            });
        ...
        */
        ...
        case P9_TGETATTR:
            ...
            self.fs.stat(path, function (err, stats) {
                ...
                req[2] = stats.mode; 
                req[3] = stats.uid; // user id
                req[4] = stats.gid; // group id
                
                req[5] = stats.nlinks; // number of hard links

                // TODOhumphd: not sure about these...
                req[6] = (0x0<<8) | (0x0); // rdev, device id low
                req[7] = stats.size; // size, low
                req[8] = self.BLOCKSIZE; // blocksize
                req[9] = Math.floor(stats.size/512+1); // blocks, low
                req[10] = stats.atime / 1000; // atime (sec)
                req[11] = stats.atime * 1000000; //0x0; // atime_nsec
                req[12] = stats.mtime / 1000; // mtime (sec)
                req[13] = stats.mtime * 1000000; // 0x0; mtime_nsec
                req[14] = stats.ctime / 1000; // ctime (sec)
                req[15] = stats.ctime * 1000000; // 0x0; // ctime (nsec)
                ...
                self.fs.stat(path, function(err, stats) {
                    ...
                    self.fs.stat(parentDirPath, function(err, stats) {
                        ...
                        entries.forEach(function(entry) {
                            console.log('entry qid', entry.qid, 'name', entry.name);
                            dataOffset += marshall.Marshall(
                                ["Q", "d", "b", "s"],
                                [
                                    entry.qid,
                        ...
                        console.log('size=', size, "offset", offset, "count", count);
        ...
        case P9_TWALK:
            ...
            function _walk(path, pathParts) {
                ...
                self.fs.stat(path, function (err, stats) {
                    ...
                    offset += marshall.Marshall(["Q"], [stats.qid], self.replybuffer, offset);
                    nwidx++;
