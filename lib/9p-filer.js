...
Virtio9p.prototype.ReceiveRequest = function (index, GetByte) {
    ...
    switch(id) {
        ...
        case P9_TSYMLINK:
            ...
            var newPath = Filer.Path.join(dirPath, name);
            var symtgt = req[2];
            // If we don't get an abspath for the target, assume it's in same dir as src
            var existingPath = Filer.Path.isAbsolute(symtgt) ? symtgt : Filer.Path.join(dirPath, symtgt);
            // TODO: deal with gid
            var gid = req[3];

            message.Debug("[symlink] existingPath=" + existingPath +", newPath=" + newPath + ", gid=" + gid); 

            self.fs.symlink(existingPath, newPath, function(err) {
                if(self.shouldAbortRequest(tag)) return;
        ...
        case P9_TREADLINK:
            ...
            // TODO: This isn't totally working:
            // "readlink: filename: cannot read link: not a symlink"
            // I must have the mode wrong for the file type bits in Filer.
