...
Virtio9p.prototype.ReceiveRequest = function (index, GetByte) {
    ...
    switch(id) {
        ...
        case P9_TSYMLINK:
            ...
            var dirPath = self.fids[dfid].path;
            var name = req[1];
            var dstPath = Filer.Path.join(dirPath, name);
            var symtgt = req[2];
            // If we don't get an abspath for the target, assume it's in same dir as src
            var srcPath = Filer.Path.isAbsolute(symtgt) ? symtgt : Filer.Path.join(dirPath, symtgt);
            var gid = req[3];

            message.Debug("[symlink] srcPath=" + srcPath +", dstPath=" + dstPath + ", gid=" + gid); 

            self.fs.symlink(srcPath, dstPath, function(err) {
                if(self.shouldAbortRequest(tag)) return;
                ...
                self.fs.stat(dstPath, function(err, stats) {
                    if(self.shouldAbortRequest(tag)) return;
                    ...
                    var qid = formatQid(dstPath, stats);
