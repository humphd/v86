    ...
    switch(id) {
        ...
        case P9_TLOPEN:
            ...
            this.fs.stat(path, function (err, stat) {
                debugger;
                // TODOhumphd: deal with errors at the 9P level...
                if(err) {
/**
 ...
 */
                ...
                req[6] = (0x0<<8) | (0x0); // device id low
                req[7] = stat.size; // size low
                req[8] = self.BLOCKSIZE;
                req[9] = Math.floor(stat.size/512+1);; // blk size low
                req[10] = stat.atime; // atime
                req[11] = 0x0;
                req[12] = stat.mtime; // mtime
                req[13] = 0x0;
                req[14] = stat.ctime; // ctime
                req[15] = 0x0;
                req[16] = 0x0; // btime
                req[17] = 0x0; 
                req[18] = 0x0; // st_gen
        ...
        case P9_TREADDIR:
            ...
            console.log('[TREADDIR] sh.ls for path', fid, offset, count, path);
            self.sh.ls(path, {recursive: false} , function(err, entries) {
                ...
                console.log('[TREADDIR] sh.ls got entries', entries);
                ...
                console.log('[TREADDIR] size = ', size);
        ...
        case P9_TATTACH: // attach - size[4] Tattach tag[2] fid[4] afid[4] uname[s] aname[s]
            ...
            this.fs.stat('/', function (err, stat) {
                // TODOhumphd: deal with errors at the 9P level...
                if(err) {
                    console.error(err);
                    return;
        ...
        case P9_TWALK:
            ...
            console.log('walk=' + walk.toString(), "path=", path);
            ...
            function _walk(path, pathParts) {
                console.log('_walk', path, pathParts);
